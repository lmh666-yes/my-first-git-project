# -*- coding: utf-8 -*-
"""更新题库工具：扫描 题库/ 文件夹中的题库文档（.md / .docx，自带答案），
...
def parse_any(path):
    """按扩展名选择解析器（md → bank_parser_md；docx → bank_parser）"""
    if path.lower().endswith(".md"):
        return bank_parser_md.parse_md(path)
    return bank_parser.parse_bank(path)
...
def main():
    ...
    files = sorted(glob.glob(os.path.join(BANK_DIR, "*.docx"))
                   + glob.glob(os.path.join(BANK_DIR, "*.md")))
    if not files:
        print("[错误] 题库文件夹中没有找到题库文档（*.md / *.docx）")
    ...
    bank, problems = parse_any(chosen)
    if not bank:
        print("\n[错误] 未能从文档解析出任何题目：")
        for p in problems[:10]:
            print("   ", p)
        print("       请确认题库文档中每题带有「**答案：…**」行（md），")
        print("       或包含“单选题/判断题”大题且带“答案：…”段落（docx）。")
        _input("\n按回车退出...")
        return
    ...
    print(f"  题数统计：{' + '.join(parts)} = {len(bank)} 题")
