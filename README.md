...
- **utils_gui 升级 v2.1**：支持四文件索引、模糊查找（子序列匹配+匹配度排序）、多关键词搜索、分类带数量、排序、复制（函数名/示例/实现源码）、跳转 VS Code（直接调用 Code.exe）、VS Code Dark+ 风格源码语法高亮（配色可用 theme.ini 自定义）、详情展示参数/返回值详细说明、列表斑马纹+选中项高亮、介绍文字亮绿色
- **准确性修复**：修正全部 1110 个函数的分类与功能描述（desc 现取自已校验的注释）、修复含花括号字符串函数的源码提取、核心算法函数（排序/查找/链表/队列等）关键步骤已添加注释
