# 01函数库 1.0.1-正式版 · UTILS C 函数库

> **版本：1.0.1-正式版**（2026-08-15）· 全量测试通过 · 跨电脑拉取即可一键部署
...
- **正式版回归验证**：完整构建 + 编译运行全部 7 个测试程序通过；3 轮压力测试全部通过；全量 **1179 项**函数无重复、注释准确
- **便携性确认**：所有脚本使用 `%~dp0`/`where gcc` + 常见路径探测自动定位 MinGW，无硬编码绝对路径，从其他电脑拉取即可一键部署
