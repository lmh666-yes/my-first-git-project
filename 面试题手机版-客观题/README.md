...
> 基于桌面版「面试题刷题软件-客观题」制作的**手机端应用**。内置题库：**面试题库 · 客观题**（单选 250 + 多选 5 + 判断 35 + 特殊题 2 = **292 题**，**解析已逐题完善**），题目配图已内嵌。
> 本版为固定题库（不做题库切换），单文件、离线可用；题库更新时在桌面版换好后重新运行 `build_web.py` 即可。
> **📦 APK 安装包已构建好**：根目录 **`面试刷题-客观题.apk`**（3.6 MB，直接发到手机安装，见「方式二」）。
...
- **🧠 进度记忆**：打开自动跳到「**连续完成题目的最后一题**」：做了 1、2、3、4，跳过 5 又做了 6、7、8 → 下次打开就在**第 4 题**（答对/答错/标记掌握都算完成）；切到错题或考试再回来则回到刚才浏览的位置
- **题目配图**：带图题（第 96、136、137 题）的原卷裁剪图已 base64 内嵌，离线也能看；**点图片可全屏查看**——打开时自动**适应屏幕并居中**，支持**单指拖动平移**、**双指捏合缩放**（以手指为中心）、**双击放大 / 还原**，底部按钮：＋放大 / －缩小 / 适应屏幕 / 1:1 / ✕关闭
