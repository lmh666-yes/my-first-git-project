...
> **📦 APK 安装包已构建好**：根目录 **`面试刷题-客观题.apk`**（3.5 MB，直接发到手机安装，见「方式二」）。
...
- **错题本（寿命机制）**：答错（任何模块）自动进入错题本；**顺序/普通考试答对不再移除错题**；每题累计错误次数永久记录；错误次数达 **3/5/7/9 或 >10** 时错题寿命 +1（初始寿命 1）
- **📝 错题考试**：错题板块内按钮进入：从错题本**随机抽 ≤10 题**（选项随机），规则同模拟考试（可暂停/交卷/题号导航/回顾）；**答对寿命 -1、归零后移除**；答错寿命不变（累计错误达阈值仍会 +1）；成绩页显示“本次清除 N 道 / 剩余 N 道”
