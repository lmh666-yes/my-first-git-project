...
**489.**（面试题4 解答题第 1 题）写宏定义 MAX，求出两个数中的较大者。（同题另见：欣诺通信面试题第 5 题"定义一个判断数大小的宏"）

**参考答案：**
```c
/* 正确的三目运算符版本：整体加括号 + 每个参数加括号 */
#define MAX(x, y)  ((x) > (y) ? (x) : (y))

/* 更严谨的版本：防止自增等副作用被求值两次（GCC 语句表达式扩展） */
#define MAX_S(x, y)  ({ __typeof__(x) _x = (x); __typeof__(y) _y = (y); _x > _y ? _x : _y; })

/* C++ 推荐：内联函数/模板，天然避免副作用与类型转换问题 */
...
```
要点：①**每个参数和整体都要加括号**——若写成 `#define MAX(x,y) x>y?x:y`，则 `MAX(a+b, c*d)` 展开成 `a+b>c*d?a+b:c*d`，因关系运算符优先级高于三目而无害，但 `MAX(a&b, c)`、`MAX(x++,y)` 等会因优先级与副作用出错（如 `MAX(i++, j)` 中较大的那个参数被自增两次）；②三目版本对**不同类型**（int/float）会按 C 的常用算术转换规则提升类型；③若确实要用宏且参数可能带副作用，用 GCC 语句表达式或 `static inline` 函数更安全；④宏不能用 `if` 语句版实现（`#define MAX(x,y) if(x>y) …` 无法作为表达式求值）。
**得分点：** ① 求出较大者且语法正确 ② 完整加括号（参数与整体）③ 加分：指出参数带副作用（自增）会被求值两次的风险，或给出 inline/模板替代方案。
**来源：** 附加面试题·面试题4 解答题第 1 题（同题另见 欣诺通信面试题第 5 题）

---

**490.**（面试题4 解答题第 2 题）编写一个程序，不使用 strcpy 函数，自己写一个函数 my_strcpy 实现字符串拷贝。
...
**491.**（面试题4 解答题第 2 题）编写一个程序，不使用 strcat 函数，自己写一个函数 my_strcat 实现字符串拼接。
...
**492.**（面试题4 解答题第 2 题）编写一个程序，不使用 strcmp 函数，自己写一个函数 my_strcmp 实现字符串比较。
...
**493.**（面试题4 解答题第 3 题）实现冒泡排序算法函数。（同题另见：欣诺通信面试题第 4 题"实现冒泡排序"）
...
```
要点：①两层循环：外层控制**轮数**（n-1 轮），内层控制**每轮比较次数**（n-1-i，因为每轮结束末尾 i 个元素已就位）；②相邻比较、逆序则交换，使较大元素逐步"冒"到末尾；③时间复杂度 O(n²)（最好情况用 swapped 优化后为 O(n)），空间 O(1)，**稳定**排序（相等元素相对次序不变，因为只在严格大于时交换）；④若要**从大到小**只需把 `>` 改成 `<`；⑤数据量大时应换用 qsort/快排（n≥数百时冒泡明显偏慢）。
**得分点：** ① 两层循环边界正确（n-1 与 n-1-i）② 相邻比较交换方向正确、排序结果正确 ③ 加分：提到 swapped 提前退出优化、稳定性或复杂度分析，或大改小方向只需换比较符。
**来源：** 附加面试题·面试题4 解答题第 3 题（同题另见 欣诺通信面试题第 4 题）

---

**494.**（笔试题3 第 1 题）请写出一种排序的算法。
...
**497.**（欣诺通信面试题第 3 题）写一个实现拷贝函数的函数接口。
...
- 第三部分（附加面试题）：**17 题** —— 编号 481~497（笔试题1 编程题 8 题、面试题4 解答题 3 题拆为 5 题、笔试题3 第 1~3 题、欣诺通信面试题第 3~5 题）
- 合计：**130 题**
- 说明：
