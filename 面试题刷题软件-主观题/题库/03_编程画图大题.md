...
**得分点：** ①三目表达式形式正确 ②整体括号+参数括号完整（缺括号要扣分）③加分：指出副作用/优先级陷阱或给出 static inline 替代。 ④合并说明：同第 489 题（附加面试题·面试题4 解答题第 1 题 + 欣诺通信面试题第 5 题），重复题已合并。
...
**得分点：** ①用 fabs(x) < eps 或区间判断（不能 == 0）②说明浮点的表示/累积误差原因 ③加分：eps 取值、相对误差。 ④变体说明：附加面试题·面试题7 第 3 题要求写出 int、bool、float、指针四种类型与零值比较的 if 语句（本题的扩展版），已收录为第 503 题，两题互为变体。
...
**得分点：** 算法正确性。 ③合并说明：同 附加面试题·面试题6 文字题第 20 题（用 DrawPixel 实现 void DrawLine(x1,y1,x2,y2)，无颜色参数），重复题已合并。
...
```
**得分点：** 重叠处理（可选）+ 逐字节拷贝。 ④合并说明：同一题在多份试卷中重复出现，已合并 —— 原第 465 题（2026 年｜请编写一个内存拷贝函数，实现 C/C++ 中 memcpy 的功能。）。
...
**409.** 请用 C 语言写一个冒泡排序。（同题另见：2024面试题大题、附加面试题·面试题4 解答题第 3 题、欣诺通信面试题第 4 题——同一题已合并）

**参考答案：**
```c
#include <stdio.h>
#define N 10               /* 数组大小可自行修改 */

void bubble_sort(int num[], int n)
{
    int i, j, t;
    int swapped;           /* 优化：若某轮没有交换说明已有序，可提前结束 */
    for (i = 0; i < n - 1; i++) {              /* 控制比较轮数 */
        swapped = 0;
        for (j = 0; j < n - 1 - i; j++) {      /* 每轮比较次数递减（-i 因为末尾已排好） */
            if (num[j] > num[j + 1]) {         /* 大的往后冒 → 从小到大 */
                t = num[j + 1];
                num[j + 1] = num[j];
                num[j] = t;
                swapped = 1;
...
```
要点：①两层循环——外层控制**轮数**（n-1 轮），内层控制**每轮比较次数**（n-1-i，因每轮结束末尾 i 个元素已就位）；②相邻比较、逆序则交换，使较大元素逐步"冒"到末尾；③时间复杂度 O(n²)（用 swapped 优化后最好情况 O(n)），空间 O(1)，**稳定**排序（只在严格大于时交换）；④若要**从大到小**只需把 `>` 改成 `<`；⑤数据量大时应换用 qsort/快排。
**得分点：** ①两层循环边界正确（n-1 与 n-1-i）②相邻比较交换方向正确、排序结果正确 ③加分：swapped 提前退出优化、稳定性或复杂度分析，或指出改方向只需换比较符。 ④合并说明：同一题在多份试卷中重复出现，已合并 —— 原第 422 题（2023面试题-3（软件试题）｜写一个函数，用冒泡排序法对整型数组进行升序排序。）；原第 459 题（2026 年｜编写程序：使用冒泡排序法排序数组。（同第 409 题，另见 2026面试题-11 第 4 题、2026面试题-20 第 ）；原第 469 题（2026 年｜使用 C/C++ 实现冒泡排序。（同第 409 题））；原第 477 题（2026 年｜（2026面试题-18）数组 `a={10,2,3,4,5,6,9,8,7,1}`，用熟悉的编程语言由小到大排序。）。
...
```
**得分点：** 三指针迭代反转。 ④合并说明：同一题在多份试卷中重复出现，已合并 —— 原第 461 题（2026 年｜编写程序：将一个链表逆序。（同第 442 题实现））。 ⑤合并说明：同 附加面试题·面试题5 第 7 题（链表反转 1→2→3→4→5 ⇒ 5→4→3→2→1），重复题已合并。
...
```
**得分点：** 符号 + 逐位累加。 ③合并说明：同 附加面试题·面试题5 第 6 题（设计函数 int atoi(char *s)：跳过前导空格、遇数字或正负号开始转换、遇非数字或 \0 结束），重复题已合并。
...
## 三、附加面试题（2026 补充试卷：编程 / 画图大题）

> **来源：** `D:\github\附加面试题` —— 笔试题1 编程题（8 题）、面试题4 解答题（3 题）、笔试题3（第 1~3 题）、欣诺通信面试题（第 3~5 题）。
> **编号：** 承接上文（401~480 已用），自 **481** 起。

---

**481.**（笔试题1 编程题第 1 题，10 分）给定一个整型变量 a，写两段代码：第一个设置 a 的 bit 5，第二个清除 a 的 bit 5。在以上两个操作中，要保持其它位不变，把下面函数补充完整：
...
void set_bit(void)
{
    a |= BIT5;          /* 置位：0 | 1 = 1，其余位与 1 或运算保持不变 */
}

void clear_bit(void)
{
    a &= ~BIT5;         /* 清零：目标位与 0 相与变 0，其余位与 1 相与不变 */
}
```
要点说明：
1. **置位**用按位或 `|=`：`a | BIT5` 把 bit5 置 1，其他位 `x|0=x` 保持不变。
2. **清位**用按位与取反 `&= ~BIT5`：`~BIT5` 生成了除 bit5 外全 1 的掩码，相与后只有 bit5 变 0。
3. 也可用 `a ^= BIT5` 做"翻转"（toggle），但题目要求的是置位/清零，不能混用。
4. 若在中断与主循环共享 a，应把 a 声明为 `volatile`；若需原子性，可借助 BSRR（GPIO）或关中断/原子指令保护。
5. 常见写法更通用：`a |= (1<<n);` 与 `a &= ~(1<<n);`，注意 `1` 若为有符号 int 而位号 ≥31 时应写作 `1u<<n`。
**得分点：** ① set_bit 用 `|=` ② clear_bit 用 `&= ~` ③ 加分：说明"其余位不变"的位运算原理或提到 `^=` 是翻转、volatile/原子性注意点。
**来源：** 附加面试题·笔试题1 编程题第 1 题

---

**482.**（笔试题1 编程题第 2 题，10 分）根据右图的 SPI Flash 连接原理图和读时序图，用你所熟悉的单片机语言，写一段读取 SPI Flash 的程序。配图：连接图（MCU P10~P13 → SPI_Flash 的 CS/MOSI(DI)/MISO(DO)/CLK）与读时序图（/CS 拉低 → 发送 8 位指令 03h → 24 位地址（高位先发）→ 然后 Data Out）。

[图：SPI Flash 连接与读时序](../imgs/q-spi-flash-timing.png)

**参考答案：**（软件模拟 SPI 读取，以 STM32 为例）
```c
/* 引脚定义：P10=CS  P11=MOSI(DI)  P12=MISO(DO)  P13=CLK（按图示连接） */
static void spi_delay(void) { /* 短延时，决定时钟频率（软件 SPI 用） */ }

static void sck_high(void) { }
static void sck_low (void) { }

/* 发送并接收 1 字节：MSB first，上升沿采样（Mode0：CPOL=0,CPHA=0） */
static unsigned char spi_rw(unsigned char dat)
{
    unsigned char i, rd = 0;
    for (i = 0; i < 8; i++) {
        sck_low();                       /* 时钟拉低：准备数据 */
        MOSI_WRITE((dat & 0x80) ? 1 : 0);/* 先发最高位（MSB first） */
        dat <<= 1;
        spi_delay();
        sck_high();                      /* 上升沿：从机采样 MOSI、主机采样 MISO */
        rd = (rd << 1) | MISO_READ();     /* 读入 MISO 位（同样 MSB first） */
        spi_delay();
    }
    sck_low();
    return rd;
...
/* 读 Flash 数据：03h 指令 + 24 位地址 + 连续读出 len 字节 */
void spi_flash_read(unsigned long addr, unsigned char *buf, unsigned int len)
{
    unsigned int i;
    CS_LOW();                            /* /CS 拉低，选中器件 */
    spi_rw(0x03);                        /* 读数据指令 03h */
    spi_rw((unsigned char)(addr >> 16)); /* 地址高字节（24 位，高位先发） */
    spi_rw((unsigned char)(addr >> 8));
    spi_rw((unsigned char)(addr));
    for (i = 0; i < len; i++)
        buf[i] = spi_rw(0xFF);           /* 主机发 dummy，移位读出数据 */
    CS_HIGH();                           /* /CS 拉高，结束操作（内部自动停止） */
}
```
要点：
1. **时序对应关系**：/CS 在整帧数据期间保持低电平，先发 8 位指令 03h，再发 24 位地址，之后每个时钟周期从 DO 读出 1 位数据（Data Out 1 连续输出，直到 /CS 抬高），因此必须**连续保证片选有效**。
2. **位序**：图中标注 `* = MSB`，说明指令、地址、数据都是**高位先发（MSB first）**，与硬件 SPI 的 `SPI_FirstBit_MSB` 对应。
3. **模式**：时序图中 CLK 空闲为低（Mode0/Mode3 需按图判断），此处按 Mode0（CPOL=0、CPHA=0，即在时钟第一个（上升）边沿采样）实现；若用硬件 SPI，则配置 `SPI_CPOL_Low + SPI_CPHA_1Edge`，预分频使 SCK 不超过 Flash 允许的最高频率。
4. **地址宽度**：03h 是标准读（Read Data）指令，地址 24 位；大容量器件可能支持 4 字节地址（Read 13h）或高速读 0Bh（需加 8 个 dummy 周期）。
5. 硬件 SPI 版本可用 `HAL_SPI_TransmitReceive()`/`SPI_I2S_SendData + 等待 TXE/RXNE`，或 DMA 批量读以提高吞吐。
**得分点：** ① 正确顺序：/CS 拉低 → 发 03h → 发 24 位地址 → 连续读数据 → /CS 拉高 ② 位序 MSB first、时钟边沿与 CPOL/CPHA 说明 ③ 加分：给出硬件 SPI 配置方式或提到 DMA 批量读/不同读指令（0Bh 高速读）。
**来源：** 附加面试题·笔试题1 编程题第 2 题

---

**483.**（笔试题1 编程题第 3 题，10 分）请列举出几种数字滤波算法，并写出相应代码。

**参考答案：** 常用数字滤波算法及参考实现（嵌入式常用、计算量小）：
```c
/* 1. 限幅滤波：两次采样差超限则取上次值，抑制突发脉冲 */
#define A_MAX  20
int limit_filter(int new_val, int old_val)
{
    return (abs(new_val - old_val) > A_MAX) ? old_val : new_val;
}

/* 2. 中值滤波：连续采样 N 次（N 为奇数）取排序中间值，去除脉冲干扰 */
#define N_MID 5
int median_filter(int (*get_adc)(void))
{
    int buf[N_MID], i, j, t;
    for (i = 0; i < N_MID; i++) buf[i] = get_adc();
    for (i = 0; i < N_MID - 1; i++)                 /* 简单冒泡排序 */
        for (j = 0; j < N_MID - 1 - i; j++)
            if (buf[j] > buf[j + 1]) { t = buf[j]; buf[j] = buf[j+1]; buf[j+1] = t; }
    return buf[N_MID / 2];                          /* 取中间值 */
}

/* 3. 递推平均滤波（滑动平均）：N 个采样点的均值，平滑随机噪声 */
#define N_AVG 8
int avg_filter(int new_val)
{
    static int buf[N_AVG];
    static int idx = 0;
    static long sum = 0;
    sum -= buf[idx];
    buf[idx] = new_val;
...
/* 4. 一阶低通（惯性）滤波：平滑缓慢变化的量，响应快且省内存 */
/*    Y(n) = a*X(n) + (1-a)*Y(n-1)，a 越小越平滑但滞后越大 */
#define ALPHA_NUM 1     /* 相当于 a = 1/8 */
#define ALPHA_DEN 8
int lpf_filter(int new_val)
{
    static int last = 0;
    last = (ALPHA_NUM * new_val + (ALPHA_DEN - ALPHA_NUM) * last) / ALPHA_DEN;
    return last;
}

/* 5. 去极值平均：去掉最大最小各 M 个后求平均，抗脉冲干扰 */
/* 6. 加权平均：对近期数据给更大权重（如 1,2,3,4,5 加权） */
/* 7. 卡尔曼滤波：带状态估计的最优滤波，适合有模型的动态系统（运算量大） */
/* 8. 防脉冲干扰平均滤波：先中值再平均，兼顾脉冲与随机噪声 */
```
选型建议：①抑制**突发脉冲**（继电器动作、静电）→ 中值/去极值/限幅；②抑制**随机噪声**（ADC 抖动）→ 递推平均/加权平均；③信号**缓慢变化且要求实时**（温度、电池电压）→ 一阶低通；④要求最优估计且算力允许 → 卡尔曼；⑤采样点越多越平滑但响应越慢，需按"信号变化速率 vs 噪声幅度"折中。
**得分点：** ① 列出≥3 种滤波算法名称与适用场景 ② 给出至少 1~2 段可直接使用的代码 ③ 加分：说明各算法的优缺点/选型依据（响应速度与平滑度权衡），或提到卡尔曼/防脉冲干扰平均。
**来源：** 附加面试题·笔试题1 编程题第 3 题

---

**484.**（笔试题1 编程题第 4 题，12 分）请给出 printf 函数的实现方法。

**参考答案：** printf 的核心是"**变参解析 + 格式化输出**"，在嵌入式里通常分成"格式化到缓冲区"和"输出到串口"两层：
```c
#include <stdarg.h>

static void uart_putc(char c) { /* 把 c 写入 UART 数据寄存器并等发送完成 */ }

/* 底层输出：字符串 / 数字，全部走 uart_putc */
static void out_str(const char *s) { while (*s) uart_putc(*s++); }

static void out_num(unsigned long v, int base, int upper, int width, char pad)
{
    char tmp[32]; int n = 0;
    const char *dig = upper ? "0123456789ABCDEF" : "0123456789abcdef";
    if (v == 0) tmp[n++] = '0';
    while (v) { tmp[n++] = dig[v % base]; v /= base; }
    while (n < width) tmp[n++] = pad;          /* 左补零/空格 */
    while (n--) uart_putc(tmp[n]);             /* 逆序输出 */
}
...
int my_printf(const char *fmt, ...)
{
    ...
    va_start(ap, fmt);
    while (*fmt) {
        if (*fmt != '%') { uart_putc(*fmt++); cnt++; continue; }
        fmt++;                                  /* 跳过 '%' */
        int width = 0;
        char pad = ' ';
        if (*fmt == '0') { pad = '0'; fmt++; }
        ...
        case 'd': case 'i': {
            int v = va_arg(ap, int);
            if (v < 0) { uart_putc('-'); cnt++; out_num((unsigned long)(-v), 10, 0, width, pad); }
            else       out_num((unsigned long)v, 10, 0, width, pad);
            break; }
        case 'u': out_num(va_arg(ap, unsigned int), 10, 0, width, pad); break;
        case 'x': out_num(va_arg(ap, unsigned int), 16, 0, width, pad); break;
        case 'X': out_num(va_arg(ap, unsigned int), 16, 1, width, pad); break;
        case 'p': out_str("0x"); out_num((unsigned long)va_arg(ap, void*), 16, 0, 8, '0'); break;
        case 'c': uart_putc((char)va_arg(ap, int)); break;
        case 's': { char *s = va_arg(ap, char*); if (!s) s = "(null)"; out_str(s); break; }
...
```
实现要点：
1. **变参机制**：靠 `<stdarg.h>` 的 `va_list/va_start/va_arg/va_end`（ARM 上参数可能在 R0~R3 与栈中，`va_arg` 按类型长度步进，所以用 `%f` 取 double、`%d` 取 int 时必须与实参类型匹配，否则取错数据）。
2. **格式解析**：状态机逐字符扫描——普通字符直接输出；遇到 `%` 解析"标志/宽度/精度/长度修饰符/转换符"（可用查表或 switch），按类型从 `va_arg` 取出实参并转换为文本。
3. **数字转字符**：反复取模取余得到逆序字符，再倒序输出；注意负数取绝对值、无符号类型不要当有符号处理、`%x` 用 0~15 映射到 0-9/a-f。
4. **嵌入式简化策略**：①如果工具链已提供 `vsnprintf`，最省事的实现是"先 `vsnprintf` 到缓冲区，再逐字节走串口"，同时保留了完整格式化能力与安全性（可限制长度避免溢出）；②重定向 `fputc`/`_write` 即可让标准 printf 走串口（但注意体积与线程安全）；③FPU 缺席时不要用 `%f`（软件浮点很占空间），可改整型放大输出（如 0.01 单位用整数）；④多任务共用 printf 需加互斥或改为"写日志缓冲 + 单独任务输出"，避免输出交叉（这正是笔试题2 第 50 题的现象）。
**得分点：** ① 说明使用变参（stdarg）与格式串解析 ② 给出可用的简化实现（至少支持 %d/%s/%c/%x）③ 加分：数字转字符串的逆序处理、负号/无符号区别，或指出可用 vsnprintf+串口重定向、多任务需互斥。
**来源：** 附加面试题·笔试题1 编程题第 4 题

---

**485.**（笔试题1 编程题第 5 题，12 分）实现一个简单的双向链表。
...
/* 初始化：带头节点/尾指针的空链表 */
void dlist_init(dlist_t *l) { l->head = l->tail = NULL; l->size = 0; }

/* 尾插 */
dnode_t *dlist_push_back(dlist_t *l, int v)
{
    dnode_t *n = (dnode_t *)malloc(sizeof(dnode_t));
    if (!n) return NULL;
    n->data = v; n->next = NULL; n->prev = l->tail;
    if (l->tail) l->tail->next = n;     /* 原尾节点指向新节点 */
    else         l->head = n;           /* 空链表：头尾都指新节点 */
    l->tail = n;
    l->size++;
    return n;
}
...
/* 头插 */
dnode_t *dlist_push_front(dlist_t *l, int v)
{
    dnode_t *n = (dnode_t *)malloc(sizeof(dnode_t));
    if (!n) return NULL;
...
/* 按值查找（返回首个匹配节点） */
dnode_t *dlist_find(dlist_t *l, int v)
{
    dnode_t *p;
...
/* 删除指定节点：重点处理 头/尾/中间 三种情况 */
int dlist_erase(dlist_t *l, int v)
{
    dnode_t *p = dlist_find(l, v);
    if (!p) return -1;
...
{
    dnode_t *p;
    for (p = l->head; p; p = p->next) printf("%d ", p->data);
    printf("\n");
    for (p = l->tail; p; p = p->prev) printf("%d ", p->data);
    printf("\n");
}
...
```
要点：①双向链表每个节点有 `prev/next` 两个指针，可**双向遍历**、已知节点时删除/插入都是 O(1)；②插入/删除必须同时维护**前后两个方向的指针**，且要单独处理"头节点"（prev 为空）与"尾节点"（next 为空）的边界；③释放时先保存 next 再 free，否则访问已释放内存；④嵌入式可改用静态数组 + 索引（无 malloc 的"链式数组"）避免内存碎片。
**得分点：** ① 节点结构含 data/prev/next ② 实现插入（头/尾）、删除、遍历 ③ 加分：正确处理头尾边界、free 前先存 next、指出已知节点删除为 O(1)，或用静态内存池替代 malloc。
**来源：** 附加面试题·笔试题1 编程题第 5 题

---

**486.**（笔试题1 编程题第 6 题，8 分）请写出 swap 函数的实现方法。

**参考答案：**
```c
/* 1. 指针版（C 标准做法）：传地址才能真正交换实参 */
void swap(int *a, int *b)
{
    int t = *a; *a = *b; *b = t;
}

/* 2. 宏版：可交换任意类型（注意括号与副作用） */
#define SWAP(a, b) do { \
        typeof(a) _t = (a); (a) = (b); (b) = _t; \
    } while (0)

/* 3. 通用字节版：交换任意类型的内存块 */
void swap_any(void *a, void *b, unsigned int size)
{
    unsigned char *pa = (unsigned char *)a, *pb = (unsigned char *)b, t;
    while (size--) { t = *pa; *pa++ = *pb; *pb++ = t; }
}

/* 4. 不用临时变量：异或交换（仅限整型，若 a、b 同址会清零，慎用） */
...
```
要点与陷阱：①C 是**值传递**，`void swap(int a,int b)` 交换的只是副本，调用后实参不变——必须传指针（或 C++ 传引用 `void swap(int &a,int &b)`）；②异或法没有临时变量，但**当两个指针指向同一变量时会把该变量清零**，因此函数入口要先判同址；另外它只适用于整型，且可读性差，工程中不推荐；③宏版要靠 do-while(0) 保证在 if/else 中作为单条语句使用，并注意不要写入带自增副作用的实参（`SWAP(i++, j++)` 行为不可控）；④若要交换结构体，注意大结构体开销与浅拷贝问题。
**得分点：** ① 给出传指针的正确实现 ② 说明值传递必须传地址（否则实参不变）③ 加分：提供宏/通用字节/异或版本之一并指出异或法的同址清零陷阱或 C++ 引用写法。
**来源：** 附加面试题·笔试题1 编程题第 6 题

---

**487.**（笔试题1 编程题第 7 题，8 分）请写出 strcpy 函数的实现方法。
...
{
    char *ret = dst;
    while ((*dst++ = *src++) != '\0')   /* 赋值表达式作为条件 */
        ;
    return ret;                          /* 返回目标首地址，支持链式调用 */
}
```
要点：①循环条件用"赋值表达式的值"判断是否已复制到 '\0'，'\0' 也会被复制过去；②必须保存 `dst` 的原始首地址作为返回值（标准 strcpy 返回 dst）；③形参 src 用 `const char *` 表明不修改源；④**不做长度检查**——若 src 比 dst 空间长会**缓冲区溢出**，工程中应改用 `strncpy(dst, src, n-1)` 或 `snprintf`，并手动补 '\0'（strncpy 在不截断时不会补结尾符）；⑤要处理 dst==src 或重叠的情况（标准 strcpy 不保证重叠，重叠应使用 memmove）；⑥注意 dst 必须指向可写内存，不能传字符串字面量（`strcpy("abc","x")` 会崩溃）。
**得分点：** ① 正确实现（含 '\0' 拷贝）② 返回目标首地址 ③ 加分：指出无长度检查的溢出风险与 strncpy/snprintf 替代、const 修饰、重叠需 memmove。
**来源：** 附加面试题·笔试题1 编程题第 7 题

---

**488.**（笔试题1 编程题第 8 题，8 分）请写出 strlen 函数的实现方法。

**参考答案：**
```c
/* 1. 指针版（推荐） */
unsigned int strlen_m(const char *s)
{
    const char *p = s;
    while (*p) p++;
    return (unsigned int)(p - s);        /* 指针相减即为长度，不含 '\0' */
}

/* 2. 计数版 */
unsigned int strlen_c(const char *s)
{
    unsigned int n = 0;
    while (s[n] != '\0') n++;
    return n;
...
/* 3. 递归版（仅作理解，实际不用：栈开销大） */
unsigned int strlen_r(const char *s)
{
    return *s ? 1 + strlen_r(s + 1) : 0;
}
```
要点：①strlen 统计到**第一个 '\0' 为止**的字符个数，**不含**结尾 '\0'（sizeof 才含）；②返回类型应为 `size_t`（标准就是 size_t，此处用 unsigned int 便于理解）；③形参加 `const char *`；④必须保证字符串以 '\0' 结尾，否则会越界访问直到碰到随机的 0（这是野指针式典型 bug）；⑤效率上指针法比下标法略优，编译器的库实现会用字对齐批量判断加速；⑥不要用递归版（深串会爆栈）；⑦与 `sizeof(数组)`/`sizeof(指针)` 的区别是高频考点：`char s[]="abc"; sizeof(s)==4, strlen(s)==3`。
**得分点：** ① 正确实现（遇到 '\0' 停止、不含 '\0'）② 说明与 sizeof 的区别 ③ 加分：指出必须保证以 '\0' 结尾否则越界、const 修饰与 size_t 返回类型。
**来源：** 附加面试题·笔试题1 编程题第 8 题

---

---

**489.**（面试题4 解答题第 2 题）编写一个程序，不使用 strcpy 函数，自己写一个函数 my_strcpy 实现字符串拷贝。
...
{
    int i;
    for (i = 0; src[i] != '\0'; i++)   /* 循环赋值 */
        dest[i] = src[i];
    dest[i] = '\0';                    /* 手动补字符串结束符！ */
    return dest;                       /* 返回赋值结果（目标首地址） */
}
...
```
要点：①循环只复制到 '\0' 之前的字符，**必须手动补 `dest[i]='\0'`**（这是下标法最容易漏的一步，漏了会导致 printf 越界读到乱码）；②返回 dest 以支持 `strcat(my_strcpy(..),..)` 之类链式调用；③若改成指针法 `while ((*dest++ = *src++));` 则 '\0' 会被一并复制，无需手动补；④不做长度检查，实际使用要确保 dest 空间足够（或用带 n 参数的版本）；⑤src 加 const 表示只读。
**得分点：** ① 逐字符复制且正确补 '\0' ② 返回目标首地址 ③ 加分：指出指针法可自动复制 '\0'、或强调无长度检查的溢出风险与 const 修饰。
**来源：** 附加面试题·面试题4 解答题第 2 题（题 2.1）

---

**490.**（面试题4 解答题第 2 题）编写一个程序，不使用 strcat 函数，自己写一个函数 my_strcat 实现字符串拼接。
...
{
    int len1 = strlen(dest);            /* 目标串已有长度，从 '\0' 处开始追加 */
    int len2 = strlen(src);
    int i;
    for (i = len1; i < len1 + len2; i++) /* 把 src 接到 dest 末尾 */
        dest[i] = src[i - len1];
    dest[i] = '\0';                      /* 重新封口！ */
    return dest;
}
...
```
要点：①先从 `dest` 找到**结尾 '\0' 的位置**（用 strlen 或自己遍历），再从这个位置开始写 src 的第一个字符，**覆盖掉原来的 '\0'**；②写完后**必须在新的末尾补 '\0'**，否则结果不是合法字符串；③最关键的是**空间检查**：`dest` 剩余容量必须 ≥ strlen(src)+1，否则缓冲区溢出（这是 strcat 最经典的漏洞来源）；④`src` 与 `dest` 若重叠属未定义行为（标准规定不得重叠）；⑤返回 dest 便于链式调用。
**得分点：** ① 从 dest 末尾开始追加并正确补 '\0' ② 返回 dest ③ 加分：指出空间不足会溢出（必须检查剩余容量）、重叠输入是未定义行为。
**来源：** 附加面试题·面试题4 解答题第 2 题（题 2.2）

---

**491.**（面试题4 解答题第 2 题）编写一个程序，不使用 strcmp 函数，自己写一个函数 my_strcmp 实现字符串比较。
...
{
    int i = 0;
    while (s1[i] == s2[i]) {              /* 字符相同则继续 */
        if (s1[i] == '\0') return 0;      /* 同时到达结尾：完全相等 */
        i++;
    }
    return (unsigned char)s1[i] - (unsigned char)s2[i]; /* >0 大 / <0 小 */
}

/* 也可用指针版（更简洁） */
int my_strcmp_p(const char *s1, const char *s2)
{
...
```
要点：①必须**逐字符比较直到出现不同或双双到达 '\0'**，不能只比较长度（长度相同内容也可能不同）；②返回值语义要和标准库一致：相等返回 0、s1>s2 返回正数、s1<s2 返回负数（**不要返回布尔值**）；③比较时应转 `unsigned char` —— 标准要求按无符号字符比较，若直接相减有符号 char 可能因溢出给出错误符号；④遇到 '\0' 的比较也包含在循环里，等价于"字典序比较"；⑤工程中比较是否相等只需判断 `==0`，注意 `if (strcmp(a,b))` 的含义是"不相等"。
**得分点：** ① 逐字符比较并在不同/结尾时返回差值 ② 返回值为 0/正/负（符合库函数语义）③ 加分：说明按 unsigned char 比较的原因、长度相同不代表内容相同、或给出指针版实现。
**来源：** 附加面试题·面试题4 解答题第 2 题（题 2.3）

---

---

**492.**（笔试题3 第 1 题）请写出一种排序的算法。

**参考答案：** 以**插入排序**为例（实现简单、小数据量/近乎有序时效率高）：
```c
void insertion_sort(int a[], int n)
{
    ...
    for (i = 1; i < n; i++) {
        key = a[i];                 /* 待插入元素 */
        j = i - 1;
        while (j >= 0 && a[j] > key) {   /* 比 key 大的元素整体后移 */
            a[j + 1] = a[j];
            j--;
        }
        a[j + 1] = key;             /* 插入到正确位置 */
...
```
要点：①把数组看成"已排序区 + 未排序区"，每次从未排序区取第一个元素插入到已排序区的合适位置；②平均/最坏时间 O(n²)，最好情况（已有序）O(n)，空间 O(1)，**稳定**；③适合小规模（n≲50）或基本有序的数据（这正是快速排序递归到小区间时切换到插入排序的原因）；④其他可选算法及一句话考点：冒泡（相邻交换、稳定 O(n²)）、选择（每轮选最值、不稳定 O(n²)）、快速排序（分治分区、平均 O(n log n)、不稳定）、归并（分治合并、稳定但需 O(n) 空间）、堆排序（O(n log n)、O(1)、不稳定）、希尔（增量插入）、计数/基数排序（非比较、整数范围小时 O(n)）；⑤工程中优先用库函数 `qsort()`/`std::sort`（内省排序，已高度优化）。
**得分点：** ① 写出一种可运行的排序实现并保证结果正确 ② 说明复杂度与稳定性 ③ 加分：说明适用场景（小规模/近乎有序），或列举多种排序并给出选型依据。 ④合并说明：同 附加面试题·面试题7 第 12 题（C 语言写个排序算法并说明其优缺点），重复题已合并。
**来源：** 附加面试题·笔试题3 第 1 题
...
```c
#define parser(cmd,payload,len) parser ##cmd(payload,len)
```

那么 `parser(a,b,c);` ①是否有问题，如有该如何修改？②实际的名称和参数是什么？

**参考答案：**
①**有问题**，而且是两个问题：
- **宏不能"递归展开"**：宏 `parser` 在自身的替换列表中又出现了标识符 `parser`，但 C 预处理器规定**同一宏在展开过程中不会再次被替换**（禁止递归），所以 `parser ##cmd` 不会变成再次调用 parser 宏；`##` 的运算发生在替换列表的展开阶段，`parser` 会原样保留并与 cmd 连接。
- **`##` 连接的是记号（token），不是宏参数值已展开的文本**：`parser ##cmd` 中，`##` 左侧的 `parser` 与右侧参数 `cmd` 会先被"参数替换"再连接，得到 `parsera`；同时 `##` 两侧如果是**空参数**会有未定义/实现相关问题（如 `parser(,b,c)`）。
因此 `parser(a,b,c);` 展开结果为：
```c
parsera(b, c);
```
即得到"函数名 `parsera` + 两个实参 b、c"——**参数从 3 个变成 2 个**，len 被吃掉了（因为替换列表里 len 只在 `parser ##cmd(payload,len)` 中作为实参出现，实际是 `payload` 和 `len` 两个参数）。
②**实际的名称和参数**：名称 = `parser` 与 cmd 值 `a` 连接后的 `parsera`；参数 = `payload`→`b`、`len`→`c`，即 `parsera(b, c)`。
③**正确/改进写法**（如需按 cmd 拼接函数名）：
```c
/* 用两级宏保证参数先展开，再连接（避免参数本身还是宏时连错） */
#define CAT_(x, y) x ## y
#define CAT(x, y)  CAT_(x, y)
#define parser(cmd, payload, len)  CAT(parser, cmd)(payload, len)
```
此时 `parser(a,b,c);` 展开为 `parsera(b, c);`（名称与参数同上，但语义清晰、可控且支持 cmd 本身是宏的情况）。
④补充要点：①`##` 连接的必须是**记号**，不能连接字符串/带空格内容；②拼接出的名字必须**已存在**（函数或变量），否则链接失败；③若目标是"按命令名调用不同函数"，更推荐用**函数指针表**（`static const struct { const char *cmd; void (*fn)(char*,int); } tbl[]`）而不是宏拼接——可检查、可调试、可遍历。
**得分点：** ① 判断"有问题"并说明原因（宏不可递归 + ## 直接连接记号、参数个数变化）② 给出展开结果 `parsera(b,c)`（名称与参数）③ 加分：给出两级 CAT 宏的正确写法，或建议改用函数指针表实现"命令→函数"派发。
**来源：** 附加面试题·笔试题3 第 2 题

---

**494.**（笔试题3 第 3 题）有以下定义：
...
a. 上述中的 `char (*A)(char*)` 表示什么意思？b. 写出 Def1、Def2、Def3 的名称定义（具体实现可留空）。

**参考答案：**
**a. `char (*A)(char*)` 的含义**：A 是一个**函数指针**（指向函数的指针），它所指向的函数特征是：**形参为一个 `char*`（字符指针），返回值为 `char`**。（注意声明里 A 必须加括号，`char *A(char*)` 就变成"返回 char* 的函数声明"了——括号是区分"函数指针"与"返回指针的函数"的关键。）
**b. 名称定义**（按结构体成员类型反推，函数签名必须与指针类型完全匹配）：
```c
/* 对应 char (*)(char*)  —— 被 A、B 两个成员用到，故必须能赋给两者 */
char Def1(char *p);
char Def2(char *p);
...
const MyDef tbl[] =
{
    {0x12, Def1, Def3},   /* 注意：原定义里第二个元素用了 Def2 作为 B，类型不匹配 */
    {0x23, Def2, Def3},
};

/* 具体实现可留空（示意） */
char Def1(char *p) { (void)p; return 0; }
char Def2(char *p) { (void)p; return 0; }
void Def3(void)    { }
```
**要点/提醒**：①原题的初始化 `{0x12, Def1, Def2}` 与 `{0x23, Def2, Def3}` **类型不匹配**——成员 A 需要 `char(*)(char*)`、成员 B 需要 `void(*)(void)`，同一行里 Def2 同时充当 A 和 B 是不可能的，正确写法应是 A/B 各对应一个符合签名的函数（如上表，或把 Def2/Def3 都写成 `void(*)(void)` 并把 A 另配函数）；②这是嵌入式常见的**"命令码 → 处理函数"跳转表**写法：`ID` 是命令码，两个函数指针分别是"带参处理"与"无参处理"回调，通过 `for` 循环遍历表按 ID 查表即可派发，比 `switch-case` 更易扩展；③查表派发示意：
```c
void dispatch(char id, char *payload)
{
    unsigned int i;
    for (i = 0; i < sizeof(MyDef)/sizeof(MyDef[0]); i++)
        if (MyDef[i].ID == id) { MyDef[i].A(payload); MyDef[i].B(); return; }
}
```
④注意：表被声明为 `const`，函数指针表放只读段更安全；函数指针赋值的函数必须**签名一致**（返回类型与参数列表都要匹配），否则是未定义行为且编译器可能只给警告。
**得分点：** ① 说明 A 是函数指针、指向"形参 char*、返回 char"的函数（并强调括号的作用）② 写出三个函数的正确原型，签名与成员类型匹配 ③ 加分：指出原题初始化存在类型不匹配、说明这是"命令码→函数"跳转表并给出查表派发写法。
**来源：** 附加面试题·笔试题3 第 3 题

---

**495.**（欣诺通信面试题第 3 题）写一个实现拷贝函数的函数接口。

**参考答案：** "拷贝函数"应给出**接口（原型）+ 实现 + 使用约束**，并说明两种常见形态：
```c
/* 形态一：字符串拷贝（以 '\0' 结束，安全版带长度限制） */
char *my_strcpy(char *dst, const char *src);                     /* 不检查长度，危险 */

char *my_strcpy_s(char *dst, const char *src, unsigned int n)    /* 推荐：限制长度 */
{
    unsigned int i = 0;
...
/* 形态二：任意内存块拷贝（大小以字节计，最通用） */
void *my_memcpy(void *dst, const void *src, unsigned int len)
{
    char *d = (char *)dst;
    const char *s = (const char *)src;
    if (!dst || !src) return NULL;
    while (len--) *d++ = *s++;            /* 若要支持重叠必须改 memmove（正/反向判断） */
    return dst;
}
```
接口设计要点：①**参数顺序与库函数保持一致**（dst 在前、src 在后），并让 src 带 `const`；②**必须带长度参数**（或断言目标容量），否则缓冲区溢出——这是所有"拷贝函数"接口设计的核心；③返回 `dst`（便于链式调用与错误判断），失败（NULL 参数 / 空间不足）时返回 NULL 或错误码；④明确**是否处理重叠**（不处理就说明调用者不得重叠，或直接提供 memmove 语义的版本）；⑤明确**拷贝语义**（浅拷贝字节 vs 深拷贝数据）以及结尾符处理（字符串必须补 '\0'，memcpy 只按字节数拷）；⑥对嵌入式还可提供"从 Flash 到 RAM""DMA 版"变体，并注意对齐与大小端（跨平台数据拷贝）。
**得分点：** ① 给出函数原型 + 实现（含长度参数的安全版）② 说明 dst/src/长度/返回值的接口约定与 const 修饰 ③ 加分：指出必须带长度以防溢出、重叠需 memmove 语义、或区分浅/深拷贝。
**来源：** 附加面试题·面试题4 欣诺通信面试题第 3 题

---

### 补充：面试题5 / 面试题6 / 面试题7（综合大题 10 题）

**496.** 用 C 写一个输入的整数、倒着输出整数的函数，要求用递归方法。（附加面试题·面试题5 第 2 题，5 分）

**参考答案：**
```c
#include <stdio.h>

/* 递归倒序输出整数：12345 → 54321 */
void print_reverse(int n)
{
    if (n < 0) {            /* 负数：先输出负号，再按正数处理 */
        putchar('-');
        n = -n;             /* 注意 INT_MIN 取负会溢出，工程上可先转 long long */
    }
...
```
**要点：** ①"先输出 `n%10`、再递归 `n/10`"决定了输出顺序是倒序；若改成"先递归、后输出"就是正序输出——这正是本题的考点；②递归出口是 `n < 10`（个位数直接输出后不再递归）；③`n = 0` 时输出 0 也正确；④负数先打印 '-' 再处理绝对值；⑤`INT_MIN`（-2147483648）取负会溢出，工程代码可用 `long long` 或先转字符串处理。
**得分点：** ①函数能编译运行、输出正确倒序 ②明确用递归（不是循环/字符串反转）③加分：处理负数与 0、提到 INT_MIN 边界。

---

**497.** 编写一个函数，返回两个字符串的最大公串！例如，"adbccadebbca" 和 "edabccadece"，返回 "ccade"。（附加面试题·面试题5 第 3 题，5 分）

**参考答案：** 最大公串即**最长公共子串**（长度最大且在两串中都连续出现的一段）。
```c
#include <stdio.h>
#include <string.h>

/* 返回最长公共子串的长度，并带出它在 s1 中的起始下标 */
int lcs_substr(const char *s1, const char *s2, int *start1)
{
    int m = (int)strlen(s1), n = (int)strlen(s2);
    int dp[300][300] = {0};          /* dp[i][j]: 以 s1[i-1]、s2[j-1] 结尾的公共子串长度 */
    int i, j, best = 0, end1 = 0;

    for (i = 1; i <= m; i++)
        for (j = 1; j <= n; j++) {
            if (s1[i-1] == s2[j-1]) {
                dp[i][j] = dp[i-1][j-1] + 1;
                if (dp[i][j] > best) { best = dp[i][j]; end1 = i; }
            } else {
                dp[i][j] = 0;        /* 子串必须连续：不相等即断链 */
            }
        }
    if (start1) *start1 = end1 - best;
    return best;
}

int main(void)
{
    const char *a = "adbccadebbca", *b = "edabccadece";
    int start, len = lcs_substr(a, b, &start);
    printf("最长公共子串：%.*s（长度 %d，位于 a[%d]）\n", len, a + start, len, start);
    return 0;
}
```
**要点与说明：** ①动态规划 `dp[i][j] = dp[i-1][j-1]+1`（字符相同）否则清零——"清零"是子串与子序列的分界；②复杂度 O(m×n) 时间、O(m×n) 空间（可只留一行做滚动数组降为 O(n)）；③**原卷示例答案有误**：按题面两个字符串实际计算，最长公共子串是 **"bccade"（长度 6）**——`s1[2..7]="bccade"`、`s2[3..8]="bccade"` 完全匹配；题面给的 "ccade" 长度只有 5，应是出题时的笔误。答题时给出能输出 `bccade` 的程序并指出这一点更严谨；④若题目额外要求"返回字符串"，可把结果复制到调用者传入的缓冲里返回。
**得分点：** ①写出 O(m×n) 的正确实现（或至少讲清 DP 状态/转移/边界）②程序设计可返回起点与长度 ③加分：指出原卷示例 "ccade" 应为 "bccade"、或说明与"公共子序列"的区别。

---

**498.** 在不调用库函数的情况下，实现 strcat()、strcmp()、strcpy()、strlen()。（附加面试题·面试题5 第 8 题，5 分）
...
/* strcpy：把 src（含 '\0'）复制到 dst，返回 dst */
...
/* strcmp：相等返回 0；s1>s2 返回正数；s1<s2 返回负数 */
int my_strcmp(const char *s1, const char *s2)
{
    while (*s1 != '\0' && *s1 == *s2) {
        s1++;
...
```
**要点：** ①`while ((*d++ = *src++))` 是"复制且顺带判断结尾"的惯用法，'\0' 也会被复制；②strcpy/strcat 返回 dst 以支持链式调用；③strcmp 的返回值语义要记牢（0/正/负，不要只写 0/-1），且比较应转 `unsigned char`（标准按无符号字符比较）；④**两个安全警示**：均不做长度检查（目标空间不足即缓冲区溢出）、src 与 dst 重叠时行为未定义（重叠用 memmove）；⑤dst 必须是可写内存，不能传字符串字面量。**关联：** 本库已有单题版本（第 417、457、487、488、489、490、491 题等），本题是同一考点的"四合一"考法；另见第 407/495 题（内存拷贝）。
**得分点：** ①四个函数全部实现且行为与标准一致（含返回值规定）②s1 加 const、'\0' 处理正确 ③加分：unsigned char 比较、strcpy 返回 dst、溢出风险提示；本题可与第 417/457/487~491 题互相印证。

---

**499.** 编写 Makefile，可以同时对当前目录和至少两个当前子目录下的文件进行编辑：创建子目录 project_1、project_2；当前目录下有 main.c；project_1 下有 example_1.c、example_2.c；project_2 下有 example_3.c、example_4.c；要求 Makefile 能同时处理这 5 个文件，并在当前目录生成可执行文件 main。（附加面试题·面试题5 第 9 题，5 分）

**参考答案：**
```makefile
CC      = gcc
CFLAGS  = -Wall -g
OBJS    = main.o project_1/example_1.o project_1/example_2.o \
          project_2/example_3.o project_2/example_4.o
TARGET  = main
...
# 子目录模式规则：子目录里的每个 .c 编译到自己目录下的同名 .o
project_1/%.o: project_1/%.c
	$(CC) $(CFLAGS) -c $< -o $@

project_2/%.o: project_2/%.c
	$(CC) $(CFLAGS) -c $< -o $@
...
.PHONY: all clean
```
**要点：** ①自动变量 `$@` 目标名、`$<` 第一个依赖、`$^` 全部依赖；②**模式规则** `project_1/%.o: project_1/%.c` 让多个源文件共用一个规则（只需写两条，5 个文件全覆盖）；③注意命令前必须是 **Tab**（不能用空格），否则 make 报 "missing separator"；④默认目标是第一个目标（这里用 `all`）；⑤工程化增强：`$(wildcard project_*/*.c)` + `$(patsubst %.c,%.o,...)` 自动收集源文件、`-MMD -MP` 自动生成头文件依赖、`make -j` 并行编译。**关联：** 本库第 448 题是另一道 Makefile 题（lib.so + target），可对照复习——**变体题**。
**得分点：** ①给出正确依赖关系（5 个 .o 链接成 main）②使用模式规则/自动变量（或明确写全规则）③加分：clean/.PHONY、`-j` 并行、patsubst/wildcard 自动收集。

---

**500.** 怎样才能检测到链表中存在循环？（限制 1：链表在只读内存区域，无法标记；限制 2：内存空间有限，无法创建足够长的数组；限制 3：链表长度任意，循环位置无法确定）尝试完成代码。（附加面试题·面试题5 第 10 题，5 分）

**参考答案：** 用 **Floyd 判圈算法（快慢指针）**：慢指针每次走 1 步、快指针每次走 2 步，若有环两指针必在环内相遇；无环则快指针先到尾。
```c
typedef struct Node { int data; struct Node *next; } Node;

/* 有环返回 1，无环返回 0；时间 O(n)，空间 O(1) */
int has_loop(const Node *head)
{
    ...
    return 0;                       /* fast 走到链表尾 → 无环 */
}
```
进阶——**找环的入口点**：
```c
const Node *loop_entry(const Node *head)
{
    const Node *slow = head, *fast = head;
    while (fast && fast->next) {
        slow = slow->next;
        fast = fast->next->next;
        if (slow == fast) {                 /* 相遇后，一指针回起点 */
            slow = head;
            while (slow != fast) {          /* 同速前进，再遇即入环点 */
                slow = slow->next;
                fast = fast->next;
            }
            return slow;
...
```
**要点：** ①可行性：有环时快指针相对慢指针每轮逼近 1 步，必在环内追上；②复杂度 O(n) 时间、**O(1) 空间**、只读链表——恰好满足题目的三条限制（不能标记、不能开数组）；③入环点原理：设头到入环点为 x、相遇点距入环点 y、环长 L，则 2(x+y)=x+y+kL ⇒ x=kL−y——"从头出发"与"从相遇点出发"同速走会在入环点相遇；④对比其它方案：改指针/打标记（需要可写内存）、哈希表或数组记录访问（需要额外空间）都不满足限制。
**得分点：** ①快慢指针思路 + 可运行代码 ②O(n) 时间与 O(1) 空间、能满足三条件 ③加分：给出找入环点的推导或代码、说明相遇必然性。

---

**501.** 用 C++ 写个程序判断一个操作系统是 16 位还是 32 位的？不能用 sizeof() 函数。（附加面试题·面试题5 第 16 题，5 分）
...
int main(void)
{
    /* 方法一：看整型的表示范围（~0u 的十进制位数/上限） */
    unsigned int umax = ~0u;
    printf("整型宽度：%s\n", (umax > 0xFFFFu) ? "≥32 位" : "16 位");

    /* 方法二：看指针（地址空间宽度，即机器字长） */
    void *p = (void *)0xFFFFu;
    unsigned long addr = (unsigned long)p;      /* 指针可无损转成足够宽的整数 */
    printf("指针宽度：%s\n", (addr > 0xFFFFul) ? "≥32 位" : "16 位");

    /* 方法三：用指针差值间接求类型大小（等价 sizeof，但没调用 sizeof） */
    int arr[2];
    long nbytes = (long)((char *)&arr[1] - (char *)&arr[0]);  /* = sizeof(int) */
    printf("int 占 %ld 字节\n", nbytes);

    return 0;
}
```
**要点：** ①"操作系统是 16/32 位"本质指 **CPU 地址宽度（字长）**——用**指针宽度**判断最名正言顺（方法二）；老教材也常用"int 是 16 位还是 32 位"作判据（方法一，在 16 位 DOS/Turbo C 时代成立）；②不用 sizeof 想求类型大小，可用**指针差值**：相邻元素地址之差就是元素字节数（方法三）；③注意现代 64 位平台：`unsigned int` 仍是 32 位（方法一会误判为 32 位），要区分 64 位应看指针宽度；④C++ 中指针转 `uintptr_t`（`<cstdint>`）更规范。
**得分点：** ①给出可行判断方法（指针宽度和/或整型范围）②说明原理：16/32 位指地址宽度，指针宽度即字长 ③加分：指针差值代替 sizeof、指出 64 位平台的判别差异。

---

**502.** 打印倒三角形，底边长 n 作为参数输入，从键盘输入。例：n=7 打印 `*******` / `*****` / `***` / `*`；n=8 打印 `********` / `******` / `****` / `**`。（附加面试题·面试题6 第 19 题，10 分）
...
int main(void)
{
    ...
    for (i = n; i >= 1; i -= 2) {            /* 每行星号数：n, n-2, n-4 …（偶数到 2） */
        for (j = 0; j < (n - i) / 2; j++)    /* 居中前导空格 */
            putchar(' ');
        for (j = 0; j < i; j++)              /* 本行星号 */
            putchar('*');
        putchar('\n');
 ...
 ******
  ****
   **
```
**要点：** ①星号数每行减 2：奇数 n 最后一行 1 个，偶数 n 最后一行 2 个；②`(n-i)/2` 个前导空格实现**居中**（题面示例是居中的）；若判卷只要左对齐三角形，去掉空格循环即可；③双层循环 O(n²)；④扩展：也可用 `printf("%*s%.*s", ...)` 的宽度技巧单行打印，或把 n 换成"参数传入"的独立函数 `void draw(int n)` 以便复用。
**得分点：** ①循环边界正确（星号规律 n,n-2,…）②居中空格逻辑（或按题目要求左对齐）③输入校验与运行效果正确 ④加分：封装成函数、说明奇偶 n 的末行差异。

---

**503.** 写出 int、bool、float、指针变量与"零值"比较的 if 语句。（附加面试题·面试题7 第 3 题，基础题）
...
/* 2) bool 与零比较：直接判断本身（不要写 == true / == 1） */
if (flag)    { /* 真 */ }
if (!flag)   { /* 假 */ }
/* 注：C99 起用 bool 需 #include <stdbool.h>；C++ 原生支持 */

/* 3) float/double 与零比较：不能直接 ==（有精度误差），用容差 */
#include <math.h>
#define EPS 1e-6                 /* 容差按数据量级取，也可用 FLT_EPSILON */
if (fabs(f) <= EPS) { /* 视作等于零 */ }
if (fabs(f) >  EPS) { /* 视作不等于零 */ }

/* 4) 指针与零比较：用 NULL（C++ 推荐 nullptr），而不是裸 0 */
if (p == NULL) { /* 空指针 */ }
if (p != NULL) { /* 非空指针 */ }
```
**要点：** ①int 直接 `== 0`；②bool 直接判断，写成 `if (flag == true)` 属于风格禁忌（且 C++ 里 bool 提升另有细节）；③**浮点不能直接等值比较**：`f == 0.0` 可能因舍入永不成立，应写成 `fabs(f) <= EPS`，比较两个浮点数大小时同理应留容差；④指针用 NULL/nullptr 表达"空"，比 `== 0` 更清晰。**关联：** 本库第 51 题是同一考点的 **float 单类型版**（经典《高质量 C/C++ 编程》原题），本题为四类型扩展版——**变体题**。
**得分点：** ①四种写法全部正确（int 直比、bool 直接判断、float 容差、指针判 NULL）②说明浮点为何不能直接比 ③加分：EPS 取值、stdbool.h/nullptr 等细节。

---

**504.** 用 C++ 实现键盘输入 10 个数，按顺序输出，并输出总和。（附加面试题·面试题7 第 19 题，15 分）
...
int main()
{
    ...
    int sum = 0;
    for (int x : a)
        sum += x;                     /* 求和（大数场景可改 long long 防溢出） */

    sort(a.begin(), a.end());         /* "按顺序"= 升序输出；若要原始输入顺序则删掉本行 */
...
```
**要点：** ①题目三部分：读入 10 个数 → "按顺序输出" → 输出总和；"按顺序"通常理解为排序后顺序输出（若指按输入顺序输出，则去掉 sort）；②`sort` 为 introsort，平均 O(n log n)；C 语言版可写冒泡/选择或调用 `qsort`；③求和注意溢出（int 对 10 个大数可能不够，可用 `long long`）；④也可用 `std::array<int,10>`、`std::accumulate` 更简洁（`#include <numeric>`）。
**得分点：** ①输入循环正确（10 个）②输出顺序符合题意（排序或原序，需自圆其说）③求和正确 ④加分：边界/溢出处理、STL 使用得当。

---

**505.** 现有 1M 长度 char 数组，请将每个字节进行高低位互换（如右图处理，n=7）。如果我们需要**频繁地**处理这样的数据，请实现一个高效的处理函数。（附加面试题·面试题7 第 20 题，15 分）

**参考答案：** "高低位互换"指每个字节的**位序整体颠倒**（bit7↔bit0、bit6↔bit1 …）。
```c
#include <stddef.h>
...
{
    b = (unsigned char)(((b & 0x55u) << 1) | ((b & 0xAAu) >> 1));  /* 相邻 1 位交换 */
    b = (unsigned char)(((b & 0x33u) << 2) | ((b & 0xCCu) >> 2));  /* 相邻 2 位交换 */
    b = (unsigned char)((b << 4) | (b >> 4));                      /* 高低 4 位交换 */
    return b;
}

/* 高效版：256 项查表 —— 每字节一次查表 + 一次写 */
static unsigned char g_rev8[256];
static void build_rev_table(void)
{
    int i;
    for (i = 0; i < 256; i++)
        g_rev8[i] = reverse_bits((unsigned char)i);
}

void reverse_buffer(unsigned char *buf, size_t len)
{
    size_t i = 0;
    for (; i + 8 <= len; i += 8) {          /* 8 字节一组：减少循环与分支开销 */
        buf[i+0] = g_rev8[buf[i+0]];
        buf[i+1] = g_rev8[buf[i+1]];
        buf[i+2] = g_rev8[buf[i+2]];
        buf[i+3] = g_rev8[buf[i+3]];
        buf[i+4] = g_rev8[buf[i+4]];
        buf[i+5] = g_rev8[buf[i+5]];
        buf[i+6] = g_rev8[buf[i+6]];
        buf[i+7] = g_rev8[buf[i+7]];
    }
    for (; i < len; i++)                    /* 收尾余数字节 */
        buf[i] = g_rev8[buf[i]];
}
```
**要点：** ①正确性：掩码交换 = 1 位交换 → 2 位交换 → 4 位交换，三步完成全字节位序反转（可先用 `0x80` 验证 0x01 应变成 0x80）；②**"频繁处理大数据"的核心优化是查表**：256 字节表常驻缓存、每字节一次查表 + 一次写，比逐位计算快数倍；③再进一步：8 字节成组、用 32/64 位字操作（字内位反转 + 字节序反转合并处理）、或 SIMD（如 SSSE3 的 `pshufb` 一次 16 字节，再配 `pmovmskb` 做位反转），嵌入式上位机按平台取舍；④表可在启动时构建一次，或直接以常量表写入代码避免运行时初始化；⑤顺序遍历对缓存友好，1M 数据一次线性扫描即可。
**得分点：** ①给出正确实现（位序颠倒，可验证）②明确"查表"这一高效方案并用复杂度/理由说明 ③加分：8 字节成组、字操作/SIMD、常量表等进一步优化，或提到调用时机（初始化一次）。
...
- 第二部分（试卷大题汇编）：**73 题** —— 编号 401~480（去重后）
- 第三部分（附加面试题）：**15 题** —— 编号 481~495（笔试题1 编程题 8 题、面试题4 解答题拆为 5 题、笔试题3 第 1~3 题、欣诺通信面试题第 3~5 题）
- 第四部分（附加面试题·面试题5/6/7）：**10 题** —— 编号 496~505
- 合计：**131 题**
- 说明：
  - 迁出题目编号沿用 `02_主观题.md` 原编号（6、7、8、29 … 386），故第一部分编号不连续。
  - 第二部分原为 1~80 号，为避免与第一部分编号冲突，统一加 400（1→401 … 80→480）。
  - 第三部分配图：SPI Flash 连接与读时序图已同步到 `imgs/q-spi-flash-timing.png`（第 482 题引用）。
  - **去重说明（2026-09-16 全库查重）**：冒泡排序一题在多份试卷重复出现，已合并到第 **409** 题（原第 422、438、459、469、477、489+493 等重复条目及其来源已在第 409、32 题处标注）；链表逆序合并到第 **442** 题（原第 461 题）；内存拷贝合并到第 **407** 题（原第 465 题）；"用宏定义判断两数大小"合并到第 **32** 题（原第 489 题）。
  - 第 482 题原文的"参考答案"字段行已改为标准格式（`**参考答案：**`），确保导入软件后能正常显示答案。
  - 2026-09-21 补充：面试题5/6/7 中大题共 14 道，其中 4 道与旧题重复已合并（面试题5 第 6 题 atoi → 第 460 题；第 7 题链表反转 → 第 442 题；面试题6 第 20 题画线 → 第 401 题；面试题7 第 12 题排序 → 第 492 题），另 10 道收录为第 496~505 题；第 51 题（float 零值比较）与新第 503 题（四种类型零值比较）为变体题互注。
